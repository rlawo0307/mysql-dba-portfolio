...
## Cost Model
* 옵티마이저가 실행 계획을 선택하기 위해 사용하는 비용 계산 기준
* 옵티마이저의 모든 실행 계획 선택의 기준이 되는 핵심 엔진
* 단순히 빠른 방법을 선택하는 것이 아니라, 각 실행 계획의 cost를 계산하여 비교 후 선택
* 계산된 cost는 통계 정보를 기반으로 한 추정값임
* 실제 실행 전에는 정확한 수치를 알 수 없음
* 통계가 틀리거나 오래됐다면 정확한 실행 계획을 선택할 수 없음
### cost model 동작 흐름
```text
1. 실행 계획 후보 생성
2. 통계를 기반으로 각 후보에 대한 row 수 추정
3. I/O 및 CPU 비용 계산
4. 총 cost 계산
5. 가장 낮은 cost의 실행 계획 선택
```
### 비용 계산 기준
* 예상 row 수
    * selectivity (선택도)
...
* CPU 비용
    ...
    * 집계 연산 등
* Back Lookup 비용
    * secondary index 사용 시 발생
    * index → pk lookup 비용 포함
<br><br>

# 통계 정보란?
* 테이블과 인덱스의 데이터 분포를 요약해 둔 메타데이터
* 옵티마이저가 cost를 계산할 때 쓰는 유일한 근거 자료
## 테이블 통계 정보 조회 → `mysql.innodb_table_stats`
```sql
...
## 인덱스 통계 정보 조회 → `mysql.innodb_index_stats`
```sql
...
* 히스토그램이 cardinality 기반 추정보다 더 정확하게 사용될 수 있음
    * 인덱스가 있다면 옵티마이저는 기본적으로 인덱스 통계(cardinality)를 사용
    * 단, 히스토그램이 존재한다면 해당 컬럼의 selectivity 추정에 히스토그램이 활용될 수 있음
    * 히스토그램은 실제 데이터 분포를 기반으로 하기 때문에 cardinality보다 더 정확한 추정이 가능한 경우가 많음 
    * 단, 히스토그램은 보조적인 통계 정보이며 항상 우선적으로 사용되는 것은 아님
    * 옵티마이저는 단순히 빠른 추정보다 cost 기반으로 더 적절한 실행 계획을 선택함
    * 최종 실행 계획은 cost 기반으로 결정됨
* 기본적으로 자동으로 생성되지 않음
### 히스토그램의 효과가 적은 경우
* 데이터 분포가 이미 균등한 경우
