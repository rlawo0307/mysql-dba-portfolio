...
### 결론
복합 인덱스에서 equality + range 조건과 order by가 함께 사용될 경우, range 조건이 인덱스 탐색 범위에 포함되는지에 따라 인덱스 정렬 사용 가능 여부가 달라진다. (**range + order by 케이스와 동일**)

복합 인덱스 (c1, c2, c3)는 c1 → c2 → c3 순서대로 정렬된 B+Tree 구조를 가지며 left-most rule에 의해 앞에서부터 연속적으로 조건을 인덱스 탐색에 사용할 수 있다. `where c1 = 10 and c2 > 20 order by c3`의 경우, 순서대로 (c1, c2)까지 인덱스 탐색에 사용할 수 있으며 (c1, c2) 범위를 기준으로 range scan을 수행한다. 이때 결과 레코드들은 서로 다른 c2 값을 기준으로 여러 그룹으로 나뉘며 각 그룹 내부에서는 c3이 정렬되어 있지만 전체 결과 집합에 대해 c3의 정렬 순서는 보장되지 않는다. 따라서 옵티마이저는 인덱스를 이용한 정렬을 사용할 수 없다고 판단하여 filesort를 수행한다.

반면 `where c1 = 10 and c3 > 30 order by c2`의 경우, 조건에 c2가 없으므로 c3을 탐색 조건으로 사용할 수 없다. 결과적으로 `c1 = 10` 조건만 인덱스로 탐색한 뒤 해당 범위를 c2 순서대로 읽는다. 이후 `c3 > 30` 조건은 인덱스 탐색이 아닌 필터 단계에서 평가되며, 이 과정에서도 c2의 정렬 순서는 유지된다.

즉, equality 조건은 인덱스 정렬을 깨지 않지만, 이후 컬럼에 range 조건이 인덱스 탐색에 포함되는 경우에는 그 이후 컬럼에 대한 정렬은 보장되지 않는다.
## order by + limit
### table 생성
```sql
set session cte_max_recursion_depth = 500000;
...
```
```sql
| -> Covering index scan on t1 using idx1  (cost=50203 rows=499550) (actual time=0.0262..167 rows=500000 loops=1)
 |
```
### order by + limit
```sql
...
```
```sql
| -> Limit: 10 row(s)  (cost=0.00499 rows=10) (actual time=0.0243..0.0313 rows=10 loops=1)
    -> Covering index scan on t1 using idx1  (cost=0.00499 rows=10) (actual time=0.0236..0.0301 rows=10 loops=1)
 |
```
### 실행 시간 결과
* order by 단독 사용 : `0.0262..167`
* order by + limit : `0.0243..0.0313`
### 결론
order by를 단독으로 사용하는 것보다 limit과 함께 사용한 경우 실행 시간이 현저하게 빠른것을 확인할 수 있다. 

order by만 사용하는 경우, 전체 결과 집합에 대해 정렬을 수행해야 하므로 많은 row를 읽고 정렬하는 비용이 발생한다. 반면 order by + limit의 경우, 옵티마이저는 필요한 상위 N개의 row만 읽으면 조기 종료되므로 전체 정렬을 수행할 필요가 없다.

즉, limit을 사용했을 때 실행 시간이 단축되는 것은 limit 자체가 빠른 것이 아니라 읽어야 하는 row 수와 정렬 비용이 크게 줄어들기 때문이다.